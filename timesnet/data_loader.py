...
class NASADataLoader(BaseDataLoader):
    ...
    def load_item(self, item_id):
        ...
        try:
            ...
            self.logger.info(f"Loading NASA data for {item_id}: {train_file}, {test_file}")
            ...
            self.logger.info(
                f"Loaded NASA {item_id}: Train={train_data.shape}, Test={test_data.shape}, Anomalies={test_labels.sum()}")
    ...
    def load_item(self, item_id):
        try:
            ...
            if timestamp_col:
                ...
                # Convert timestamp to numeric feature (seconds since Unix epoch)
                for df in [train_df, test_df]:
                    df[timestamp_col] = pd.to_datetime(df[timestamp_col])
                    df[timestamp_col] = df[timestamp_col].astype('int64') // 10 ** 9  # Convert to Unix timestamp
            else:
