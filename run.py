...
if __name__ == '__main__':
    ...
    # If you want to restrict visible GPUs, set CUDA_VISIBLE_DEVICES first:
    if args.use_multi_gpu:
        # e.g. "--devices 0,1,2"
        os.environ["CUDA_VISIBLE_DEVICES"] = args.devices.replace(' ', '')
        device_ids = list(range(len(args.devices.split(','))))
        args.device_ids = device_ids
        # the first listed device becomes default cuda:0
        args.primary_gpu = 0
    else:
    ...
    if torch.cuda.is_available():
        ...
        print(f"GPU available: {torch.cuda.get_device_name(args.primary_gpu)}")
        print(f"Total GPUs: {torch.cuda.device_count()}")
        print(f"Primary GPU: cuda:{args.primary_gpu}")
    else:
        ...
        if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            ...
            args.device = torch.device("cpu")
        print("CUDA not available — using", args.device)

    print('Args in experiment:')
    print_args(args)

    ExpClass = Exp_Anomaly_Detection

    def run_experiment(is_training, iteration):
        # instantiate experiment
        exp = ExpClass(args)
        # move model to device and wrap for multi-GPU
        exp.model.to(args.device)
        if args.use_multi_gpu and torch.cuda.device_count() > 1:
            exp.model = torch.nn.DataParallel(exp.model, device_ids=args.device_ids)

        # build a unique setting name
        setting = (
            ...
            f"_eb{args.embed}_dt{args.distil}_{args.des}_{iteration}"
        )
